...
const { mocks } = vi.hoisted(() => {
  const config = {
    externalUsageService: { configured: true, serviceBase: 'http://usage.local' },
  };
  return {
    mocks: {
      config,
...
vi.mock('@/stores', () => ({
  ...
  useConfigStore: (
    selector: (state: {
      config: { externalUsageService: { configured: boolean; serviceBase: string } };
      fetchConfig: () => Promise<{
        externalUsageService: { configured: boolean; serviceBase: string };
      }>;
    }) => unknown
  ) =>
    selector({
...
beforeEach(() => {
  ...
  mocks.fetchConfig.mockResolvedValue(mocks.config);
